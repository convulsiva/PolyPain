...
def register_admin_handlers(bot: TeleBot):
    ...
    def on_admin_action(call: types.CallbackQuery):
        ...
        if action == "stats":
            ...
            msg = bot.send_message(
                call.message.chat.id,
                "Введите <b>ID пользователя</b>, которого хотите лишить прав администратора (или /cancel):",
                parse_mode="HTML"
            )
    ...
    def _handle_add_admin_id(message: types.Message):
        ...
        try:
            ...
            if admin_service.add_admin(target_user_id):
                bot.reply_to(message, f"✅ Пользователь <code>{target_user_id}</code> успешно назначен администратором.",
                             parse_mode="HTML")
            else:
                bot.reply_to(message, f"⚠️ Пользователь <code>{target_user_id}</code> уже является администратором.",
                             parse_mode="HTML")
        except ValueError:
        ...
        try:
            ...
            if admin_service.remove_admin(target_user_id):
                bot.reply_to(message,
                             f"✅ Пользователь <code>{target_user_id}</code> больше не является администратором.",
                             parse_mode="HTML")
            else:
                bot.reply_to(message,
                             f"⚠️ Пользователь <code>{target_user_id}</code> не был найден в списке администраторов.",
                             parse_mode="HTML")
        except ValueError:
            msg = bot.reply_to(message, "❗️Неверный ID. Пожалуйста, введите число или /cancel:")
            bot.register_next_step_handler(msg, _handle_remove_admin_id)
